...
($root = "/" . $ENV{"ARCH_FW_ROOT"}) =~ s/\/$//;
$root =~ s/^\/\//\//g;
$uclibc = $ENV{"ARCH_TOOLCHAIN"};
$router = $ENV{"PORTAGE_TMPDIR"}; #TODO: I'm pretty sure this needs to be fixed 
...
#mysql - bwq518
	...
	#fixDynDep("libiptc.so.0", "xtables-multi");
	#fixDynDep("libip4tc.so.0", "xtables-multi");
	#fixDynDep("libip6tc1.so.0", "xtables-multi");
